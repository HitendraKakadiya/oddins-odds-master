...
interface LeagueCleanSheetTableProps {
  standings: StandingsRow[];
}

export default function LeagueCleanSheetTable({ standings }: LeagueCleanSheetTableProps) {
  ...
  return (
    <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="bg-brand-emerald px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <h3 className="text-xl font-black text-white uppercase tracking-widest italic">Clean Sheet</h3>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-white border-b border-slate-50">
              ...
              <th className="px-4 py-5 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Clean Sheet</th>
              <th className="px-4 py-5 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Percentage</th>
            </tr>
          </thead>
            ...
            {sorted.map((row, idx) => {
              ...
              return (
                <tr key={`${row.team.id}-${idx}`} className={`hover:bg-slate-50/50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                  ...
                  <td className="px-4 py-4 text-center text-xs font-black text-slate-800">{cs.count}</td>
