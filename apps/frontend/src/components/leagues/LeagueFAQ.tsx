...
export default function LeagueFAQ({ leagueName, faqs }: LeagueFAQProps) {
  return (
    <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-8 md:p-12 mb-12">
      <h2 className="text-3xl font-black text-brand-midnight mb-10 tracking-tight">
  ...
  return (
    <div 
      className={`border rounded-[32px] transition-all duration-300 ${
        isOpen ? 'bg-brand-emerald/5 border-brand-emerald/20 shadow-sm' : 'bg-white border-slate-100 hover:border-slate-200'
      }`}
    >
      <button 
      ...
      >
        <span className={`text-sm md:text-base font-black transition-colors ${
          isOpen ? 'text-brand-emerald' : 'text-slate-800 group-hover:text-brand-emerald'
        }`}>
          {question}
        </span>
        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
           isOpen ? 'bg-brand-emerald text-white rotate-180' : 'bg-slate-50 text-slate-400'
        }`}>
