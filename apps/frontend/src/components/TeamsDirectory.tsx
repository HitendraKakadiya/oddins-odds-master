            ...
            <div key={league.id} className="flex flex-col">
              <button 
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                  openLeagues.includes(league.id) ? 'bg-brand-emerald/5' : 'bg-slate-50/50 hover:bg-slate-50'
                }`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col items-start translate-y-[-1px]">
                    ...
                    <span className={`text-sm font-black transition-colors ${openLeagues.includes(league.id) ? 'text-brand-emerald' : 'text-slate-700'}`}>
                      {league.name}
                    </span>
                  </div>
                </div>
                ...
                {loadingTeams[league.id] ? (
                  <div className="w-5 h-5 border-2 border-brand-emerald/10 border-t-brand-emerald rounded-full animate-spin"></div>
                ) : (
                  <svg 
                  ...
                  {teamsByLeague[league.id].length === 0 ? (
                    ...
                    teamsByLeague[league.id].map((team) => (
                      <div key={team.id} className="group flex items-center justify-between p-4 hover:bg-slate-50 transition-all rounded-xl cursor-pointer">
                        <div className="flex items-center gap-4">
                          <button className="text-slate-200 hover:text-brand-emerald transition-colors">
                          ...
                          <Link href={`/team/${team.slug}`} className="text-sm font-bold text-slate-600 group-hover:text-brand-emerald transition-colors">
                            {team.name}
                          </Link>
                        </div>
                        <div className="flex items-center gap-4">
                          ...
                          <svg 
                            className="w-4 h-4 text-slate-200 group-hover:text-brand-emerald transition-all transform group-hover:translate-x-1" 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                          >
