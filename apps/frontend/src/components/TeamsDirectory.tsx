...
export default function TeamsDirectory() {
  ...
  const [searchResults, setSearchResults] = useState<{ teams: Team[]; leagues: League[] } | null>(null);
  ...
  const handleSearch = async (query: string) => {
    ...
    setSearching(true);
    try {
      const data = await fetchAPI<{ teams: Team[]; leagues: League[] }>(`/v1/search?q=${encodeURIComponent(query)}`);
  ...
  return (
    <div className="flex flex-col gap-10">
      {/* Search Header - Keep it as a power feature */}
      <div className="bg-brand-midnight rounded-[32px] p-8 text-white relative overflow-hidden shadow-2xl border border-white/5">
        <div className="relative z-10 flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h2 className="text-2xl font-bold tracking-tight">Global Search</h2>
            <p className="text-slate-400 font-medium text-sm">Find any club or league worldwide</p>
          </div>
          
          <div className="relative group">
            <div className="relative flex items-center bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-2 transition-all group-focus-within:border-brand-emerald group-focus-within:bg-white/15">
              ...
              <input 
                type="text"
                placeholder="Search..."
                className="w-full bg-transparent border-none focus:ring-0 text-white placeholder-slate-500 font-bold ml-3 py-2 text-sm"
                value={searchQuery}
      ...
      <div className="flex flex-col gap-6">
        <h2 className="text-2xl font-bold text-slate-900 border-b border-slate-100 pb-4">
          Football Teams Statistics for All Countries
        </h2>
          ...
          <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {searchResults.teams.length > 0 && (
              <div className="flex flex-col gap-4">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Teams Found</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {searchResults.teams.map((team) => (
                    <Link 
                      key={team.id}
                      href={`/team/${team.slug}`}
                      className="flex items-center gap-4 p-4 rounded-xl bg-white border border-slate-100 hover:border-brand-emerald transition-all group"
                    >
                      <img src={team.logoUrl} alt={team.name} className="w-10 h-10 object-contain" />
                      <span className="text-sm font-bold text-slate-800 group-hover:text-brand-emerald transition-colors">{team.name}</span>
                    </Link>
                  ))}
                </div>
              </div>
