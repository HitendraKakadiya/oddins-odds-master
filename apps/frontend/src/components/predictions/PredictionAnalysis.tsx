    ...
    <div className="flex flex-col gap-8">
       ...
       <div className="flex flex-col">
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6 bg-slate-50 self-start px-4 py-2 rounded-full border border-slate-100/60 shadow-inner">
             <Link href="/" className="hover:text-brand-emerald transition-colors font-black">Home</Link>
             <span className="text-slate-200">/</span>
             <Link href="/predictions" className="hover:text-brand-emerald transition-colors font-black">Predictions</Link>
             <span className="text-slate-200">/</span>
             <span className="text-brand-emerald font-black">{match.league.name}</span>
          </div>
          
          <h1 className="text-3xl md:text-5xl font-black text-brand-midnight mb-8 leading-tight tracking-tight">
          ...
          <div className="space-y-6">
             {predictions && predictions.length > 0 ? (
                predictions.map((pred, index) => (
                   <div key={index} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 flex flex-col md:flex-row gap-6 relative overflow-hidden group hover:border-brand-emerald/20 transition-all duration-300">
                      <div className="absolute top-0 left-0 w-2 h-full bg-brand-emerald opacity-80"></div>
                         ...
                         {pred.confidence && (
                           <div className="mt-4 pl-10 flex items-center gap-2">
                              ...
                              <div className="flex gap-1">
                                 {[...Array(5)].map((_, i) => (
                                    <div key={i} className={`w-3 h-3 rounded-full ${i < (pred.confidence || 0) / 20 ? 'bg-brand-emerald' : 'bg-slate-100'}`}></div>
                                 ))}
                              </div>
                           </div>
                         )}
