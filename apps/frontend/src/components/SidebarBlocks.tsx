    ...
    <div className="bg-gradient-to-br from-[#6366F1] to-[#4F46E5] rounded-[28px] p-4 text-white overflow-hidden relative shadow-xl shadow-brand-indigo/30 border border-white/20 w-full">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-lg mb-4 border border-white/30 text-slate-900">
        ...
        <div className="flex items-center justify-between gap-1 mb-4 mt-4">
          <div className="flex flex-col items-center text-center w-[100px]">
            ...
            <div className="text-[11px] font-black text-slate-800 leading-tight line-clamp-1">
      ...
      <div className="flex flex-col items-center gap-3">
         <div className="flex items-center gap-4">
            <button onClick={prevSlide} className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:bg-white/10 transition-all active:scale-90">
            ...
            <div className="flex flex-col items-center gap-2">
               ...
               <div className="flex items-center gap-1.5">
                  {displayPredictions.map((_, idx) => (
                     <button key={idx} onClick={() => setCurrentSlide(idx)} className={`h-1.5 rounded-full transition-all border border-white/10 ${idx === currentSlide ? 'w-6 bg-white' : 'w-1.5 bg-white/20'}`} />
                  ))}
               </div>
            </div>
            <button onClick={nextSlide} className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:bg-white/10 transition-all active:scale-90">
          ...
          {streams.length > 0 ? (
            streams.map((stream) => (
              <div key={stream.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  <div className="w-8 h-8 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-center text-sm shadow-sm font-bold">
                    {stream.icon}
                  </div>
                  <div>
            ...
            <div className="p-8 text-center text-xs text-slate-400 font-bold">No live events now</div>
          )}
      </div>
    </div>
  );
            ...
            <div key={group.country.name}>
               {group.leagues.map((league) => (
                  <Link key={league.id} href={`/leagues/${group.country.name.toLowerCase()}/${league.slug}`} className="flex items-center justify-between p-4 px-5 hover:bg-slate-50 transition-all border-b border-slate-100 last:border-0 group">
                    ...
                    <svg className="w-4 h-4 text-slate-300 group-hover:text-slate-500 transition-all group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7-7" /></svg>
                  </Link>
               ))}
            </div>
          ))
          ...
          <div className="divide-y divide-slate-100">
            {competitionsData.map((group) => (
              <div key={group.country.name} className="flex flex-col">
                <button onClick={() => toggleCountry(group.country.name)} className="w-full flex items-center justify-between p-5 hover:bg-slate-50 transition-all group">
                  <div className="flex items-center gap-4">
