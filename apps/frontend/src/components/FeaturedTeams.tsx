...
export default function FeaturedTeams({ initialTeams = [] }: { initialTeams?: Team[] }) {
  ...
  return (
    <section className="mt-0">
      <div className="card !p-0 overflow-hidden shadow-sm !border-slate-200/60 border-t-4 !border-t-brand-emerald bg-white">
        <div className="p-5 border-b border-slate-100 flex items-center gap-3">
          <div className="w-8 h-8 bg-brand-emerald/10 rounded-lg flex items-center justify-center text-brand-emerald">
            ...
            {teams.map((team) => {
              ...
              return (
                <Link 
                  key={team.id}
                  href={`/team/${team.slug}`}
                  className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-100 hover:border-brand-emerald hover:shadow-md transition-all group cursor-pointer select-none"
                >
