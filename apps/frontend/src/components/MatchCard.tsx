...
export function MatchRow({ match }: MatchProps) {
  ...
  return (
    <div className="bg-white p-6 flex flex-col gap-5 group hover:bg-slate-50/40 transition-all border-b border-slate-100/60 last:border-0">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="text-slate-400 text-[11px] font-black w-10 tracking-tighter">
            {kickoffTime}
          </div>
          <div className="flex items-center gap-10 min-w-[350px] justify-center">
            ...
            <div className="flex items-center gap-4 font-black text-slate-900 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100/60 shadow-inner">
      ...
      <div className="flex items-center gap-4 pl-16">
        ...
        <div className="flex gap-2">
            ...
            <button className="bg-slate-50 text-slate-700 font-black py-2.5 px-5 rounded-xl text-[10px] uppercase tracking-widest hover:bg-slate-100 border border-slate-200/60 transition-all shadow-sm">Stats</button>
        </div>
      </div>
    </div>
  );
...
export function LeagueGroup({ leagueName, country, matches }: { leagueName: string; country: string; matches: any[] }) {
  const [isOpen, setIsOpen] = useState(true);

  return (
    <div className="card !p-0 overflow-hidden mb-8 !border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
      <div 
        className="bg-slate-50/50 p-6 flex items-center justify-between border-b border-slate-100 cursor-pointer hover:bg-slate-100/50 transition-colors"
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center gap-4">
          <div className="p-2.5 bg-brand-indigo/10 rounded-xl shadow-inner border border-brand-indigo/5">
            <svg className="w-5 h-5 text-brand-indigo" fill="none" stroke="currentColor" viewBox="0 0 24 24">
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-sm text-slate-900 uppercase tracking-widest">{leagueName}</h3>
              <span className="text-[10px] bg-slate-200/50 text-slate-500 px-2 py-0.5 rounded-full font-bold">{matches.length}</span>
            </div>
            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">{country}</div>
          </div>
        </div>

        <button 
          className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200/60 text-slate-400 hover:text-brand-indigo hover:border-brand-indigo/30 transition-all shadow-sm active:scale-95"
          onClick={(e) => {
            e.stopPropagation();
            setIsOpen(!isOpen);
          }}
