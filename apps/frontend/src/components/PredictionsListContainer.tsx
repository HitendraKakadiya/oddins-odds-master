      ...
      {hasMore && (
        <div className="p-8 border-t border-slate-50 bg-slate-50/30 flex justify-center">
          <button 
            onClick={loadMore}
            disabled={loading}
            className="bg-white text-brand-emerald border-2 border-brand-emerald/10 hover:border-brand-emerald px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all hover:bg-brand-emerald/5 shadow-lg shadow-brand-emerald/5 disabled:opacity-50 disabled:cursor-not-allowed group flex items-center gap-3"
          >
            {loading ? (
              <>
                <div className="w-4 h-4 border-2 border-brand-emerald/20 border-t-brand-emerald rounded-full animate-spin"></div>
                <span>Loading...</span>
              </>
            ) : (
