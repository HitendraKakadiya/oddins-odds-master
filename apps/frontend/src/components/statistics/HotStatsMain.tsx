...
interface HotStatsMainProps {
  initialMatches: HotStatMatch[];
  initialLeagues: Array<{ id: number; name: string }>;
  initialMarket: string;
  initialDate: string;
  initialTotal: number;
}

export default function HotStatsMain({ 
  initialMatches, 
  initialLeagues, 
  initialMarket, 
  initialDate,
  ...
  const [matches, setMatches] = useState<HotStatMatch[]>(initialMatches);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState(searchParams.get('sort') || 'prob_high');
  const [selectedLeague, setSelectedLeague] = useState(searchParams.get('league') || 'all');
  const [leagues, setLeagues] = useState(initialLeagues);
  const [page, setPage] = useState(1);
  const [hasMore, setHasMore] = useState(initialMatches.length === 12 && initialMatches.length < initialTotal);
  ...
  // Sync state with URL when filters change internally
  const updateURL = (market: string, date: string, sort: string, league: string) => {
  ...
  const handleMarketChange = (id: string) => {
    setActiveMarket(id);
    updateURL(id, selectedDate, sortBy, selectedLeague);
  };

  const handleDateChange = (dateType: string) => {
    setSelectedDate(dateType);
    updateURL(activeMarket, dateType, sortBy, selectedLeague);
  };

  const handleSortChange = (val: string) => {
    setSortBy(val);
    updateURL(activeMarket, selectedDate, val, selectedLeague);
  };

  const handleLeagueChange = (id: string) => {
    setSelectedLeague(id);
    updateURL(activeMarket, selectedDate, sortBy, id);
  };

  useEffect(() => {
    async function fetchStats() {
      if (page === 1 && matches === initialMatches && activeMarket === initialMarket && selectedDate === initialDate) return;
    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="bg-slate-50/50 p-2 rounded-[28px] border border-slate-100 mb-8 flex items-center w-fit overflow-x-auto">
          ...
          <button 
            onClick={() => handleDateChange('after-tomorrow')}
            className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex flex-col items-center gap-0.5 ${
